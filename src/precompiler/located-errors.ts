...
export class LocatedErrors {
    #throwError(at: AnyNode, msg: string | (() => string), errorTop: Function): never {
        let msgString = typeof msg === 'string' ? msg : msg()
        let err

        if (at?.loc?.source) {
            err = Error(msgString + ' (' + at.loc.source + ':' + at.loc.start.line + ':' + at.loc.start.column + ')')
        } else if (at?.loc) {
            err = Error(msgString + ' (' + at.loc.start.line + ':' + at.loc.start.column + ')')
        } else {
            err = Error(msgString)
        }
    ...
    invariantAt(node: AnyNode, cond: any, msg: string | (() => string)): asserts cond {
        if (!cond) {
