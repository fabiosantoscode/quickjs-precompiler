...
import { astNaiveChildren, astTopLevelChildren } from "./ast-traversal";
import { AnyNode, Function, Identifier, Pattern, Program } from "./augmented-ast";
...
class UniqueifyVisitor extends LocatedErrors {
    ...
    scopes = [
        { type: 'var' as 'var' | 'let', variables: new Map<string, string>(), preventReassign: new Set(), isGlobal: true }
    ]
    ...
    visitNode(node: AnyNode) {
        switch (node.type) {
            case "Identifier": {
                invariant(!node.uniqueName, 'must be the first time we see this ident/label')
            ...
            case "BreakStatement": {
                if (node.label) {
                    ...
                    this.invariantAt(node.label, label, () => 'unknown label ' + node.label?.name)
            ...
            case "VariableDeclaration": {
                for (const decl of node.declarations) {
                    ...
                    this.uniqueifyDeclaration(decl.id, node.kind === 'var' ? 'var' : 'let')
                }
                return
            }
            // Functions
            ...
            case "AssignmentExpression": {
                ...
                for (const assignee of this.findPatternAssignees(node.left)) {
                    let scope = this.scopes.findLast(scop => scop.variables.has(assignee.name))
                    if (scope?.preventReassign.has(assignee.name)) {
                        this.borkAt(assignee, 'Cannot reassign (is this the name of a FunctionDeclaration?)')
                    }
                }
    ...
    *findPatternAssignees(pattern: Pattern): Iterable<Identifier> {
        invariant(pattern.type === 'Identifier')
        ...
        function exploreScope(node: AnyNode) {
            if (node.type === 'VariableDeclaration' && node.kind === 'var') {
                for (const declarator of node.declarations) {
                    invariant(declarator.id.type === 'Identifier', 'destructuring not supported')
    ...
    uniqueifyDeclaration(id: Pattern, declKind: 'var' | 'let') {
        ...
        this.invariantAt(id, uniqueName, () => `missing variable ${uniqueName}. Variables must be predeclared using ${this.prepareLetScoped.name} and ${this.prepareVarScoped.name}`)
