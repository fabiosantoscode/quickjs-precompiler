...
it('unless they are in the same scope', () => {
    expect(testUniqueNames(`let x = 1, y = x + 1`)).toMatchInlineSnapshot(`"let x@1 = 1, y@1 = x@1 + 1;"`)
})
...
it('supports weird hoisted functions', () => {
    expect(testUniqueNames(`function x() { inLet; } let inLet;`)).toMatchInlineSnapshot(`
        "function x@1() {
          inLet@1;
        }
        let inLet@1;"
...
it('breaks with unsupported weird cases', () => {
    expect(() => testUniqueNames(`foo(); foo = () => {}; foo(); function foo() { return foo }`)).toThrow(/reassign/i)
    expect(() => testUniqueNames(`function foo() { foo = 2 }`)).toThrow(/reassign/i)
